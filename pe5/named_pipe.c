  ...
  if (fifo == 0) {
    ...
    while (true) {
      if (child_pid == 0) {
        ...
        ssize_t bytes_written = write(named_fd, block, block_size);
        ...
        ssize_t bytes_read = read(named_fd, buffer, block_size);
